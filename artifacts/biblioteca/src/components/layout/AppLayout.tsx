...
import { BookOpen, Users, LayoutDashboard, History, BookMarked, Sun, Moon, LogOut, Menu, X, GraduationCap } from "lucide-react";
...
export function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  const isPedagogo = user?.role === "pedagogo";
  ...
  const pedagogoLinks = [
    { href: "/pedagogo", label: "Dashboard", icon: LayoutDashboard },
    { href: "/pedagogo/books", label: "Acervo de Livros", icon: BookOpen },
    { href: "/pedagogo/loans", label: "Empréstimos", icon: History },
  ];
  ...
  const links = isAdmin ? adminLinks : isPedagogo ? pedagogoLinks : studentLinks;
  ...
  const SidebarContent = () => (
    <>
      ...
      {isPedagogo && (
        <div className="mx-4 mt-4 flex items-center gap-2 px-3 py-2 rounded-xl bg-emerald-500/10 border border-emerald-500/20">
          <GraduationCap className="w-4 h-4 text-emerald-600 dark:text-emerald-400 shrink-0" />
          <span className="text-xs font-semibold text-emerald-700 dark:text-emerald-300">Área do Pedagogo</span>
        </div>
      )}
      ...
      <div className="p-4 border-t border-border/50">
        <div className="flex items-center gap-3 px-4 py-3 bg-muted/50 rounded-xl mb-4 border border-border/50">
          ...
          <div className="overflow-hidden">
            ...
            <p className="text-xs text-muted-foreground truncate">{roleLabel[user?.role || ""] || user?.role}</p>
          </div>
        </div>
